...
- `src/content/blog/beste-nettside-leie-lokale-hytte-utstyr-norge.md` — the
  ...
  `date`, `updated` (was misspelled `lastUpdated` in this branch's starting
  state — a dead field never read by any parser), `keywords`, and
  `schema: "FAQPage"` + `faqQuestion`/`faqAnswer` (cosmetic; the actual
  FAQPage JSON-LD is driven by `src/content/blogFaq.mjs`, not these two
  fields — confirmed unchanged by this branch, still passes
  `blogFaq.test.ts`).
...
## SCOPE
- **In scope:** frontmatter (`title`, `description`, `updated`) and body
  copy of one markdown file,
  `src/content/blog/beste-nettside-leie-lokale-hytte-utstyr-norge.md`.
- **Out of scope:** `scripts/prerender.mjs`, `src/entry-server.tsx`,
  `pnpm-workspace.yaml`, `blogFaq.mjs`, any other blog post.
- **Acceptance criteria:** sharper title/meta description that names a
  concrete number and an explicit next action; intro and a new "I korte
  trekk" box surface concrete, sourced numbers; two new internal links to
  live routes; closing CTA links to a live route; dead `lastUpdated` field
  fixed to `updated` so `dateModified` reflects the real edit date; full
  test suite and `tsc --noEmit` stay green; static prerender picks up the
  same fields via its independent parser.
...
`prepare_attachment_upload`/`create_attachment_from_upload`-equivalent tool
exists). This spec is committed to the branch per the escalation path but
