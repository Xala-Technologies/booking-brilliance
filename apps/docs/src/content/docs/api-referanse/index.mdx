...
V1-dokumentasjonen dekker public + admin. Platform-API-et (`/api/v1/`) har nå betalingsmetode-endepunkter for Digilist Mobile — lagrede kort som kan belastes direkte ved checkout, og Vipps-betalingsavtaler (satt opp/aktivert/stoppet via REST, ikke lenger stub) — dokumentert i `GET /api/v1/openapi.json`. Full endepunkt-referanse i denne seksjonen kommer senere.
