...
**Lens:** is anything in this diff NOT the stated change? Read the full
five-file diff (`git diff origin/main...HEAD --stat`), checked the blog
`.md` file's diff hunk-by-hunk against SPEC.md's "WHAT CHANGES" list, and
then looked hard at the two files that aren't blog content or `.agent/`
review docs: root-level `AGENT-GOAL.md` and `AGENT-SPEC.md`.

**Checked:**

- `beste-nettside-leie-lokale-hytte-utstyr-norge.md`: every hunk maps
  1:1 to a bullet in SPEC.md §3 (title, description, intro reframe, "I
  korte trekk" box, two internal links, CTA link, `lastUpdated`→`updated`
  fix). Nothing extra — no unrelated paragraph edits, no touched
  frontmatter fields outside `title`/`description`/`updated`, no
  reflowed/reformatted lines beyond the diff's own edits.
- `.agent/XAL-1161/SPEC.md` and `REVIEW.md`: expected artefacts for this
  ticket's own documentation trail, written by this branch's own earlier
  steps — in scope by the process this branch is following.
- `AGENT-GOAL.md` (added by `ac683cd`, pre-existing before this session):
  auto-generated by the fleet's `prepare.ts` when the branch was cut
  (confirmed via `tools/improvements-agent/src/prepare.ts:86-108`, which
  writes and `git add`s this exact file at branch-prep time). The
  implementing agent's own prompt template
  (`tools/improvements-agent/src/implement.ts:34`) already instructs
  "Delete AGENT-GOAL.md before the PR" as a step owned by whichever
  session opens the PR — not this review round. Left as-is; deleting it
  here would be reaching past this round's job and duplicating a step the
  PR-opening prompt already owns (confirmed via the XAL-1163 precedent,
  where "chore: delete AGENT-GOAL.md before opening PR" landed as its own
  commit right before that ticket's PR, not during its review rounds).
- **`AGENT-SPEC.md` (added by `af9f7c3`, this branch, previous round) —
  real finding.** Traced its history: `main` carried a root-level
  `AGENT-SPEC.md` (leftover from XAL-1163's merge), then `main` commit
  `15c7b14` ("chore: remove agent scaffolding from main", merged into this
  branch via `94b6319` at `ac683cd`+7min) deleted it outright, with an
  explicit policy message: *"AGENT-SPEC.md was tracked on main, so every
  agent branch that dropped its own copy conflicted as modify/delete — PRs
  #236 and #237 were both blocked on exactly this and nothing else. These
  are per-run working files, not repository content. Artefacts now live in
  `.agent/<ISSUE>/`."* This branch's merge commit (`94b6319`) correctly
  picked up that deletion. Ten minutes later, `af9f7c3` ("backfill
  AGENT-SPEC.md — step 0 was skipped this branch") re-added a
  near-duplicate of it — recreating, on this branch, the exact file/exact
  problem `main` had just deleted for the exact reason cited (root-level
  `AGENT-SPEC.md` is superseded by `.agent/<ISSUE>/SPEC.md`, which this
  branch already has, content-identical). Grepped `tools/` for any
  remaining reference to `AGENT-SPEC.md`: none — unlike `AGENT-GOAL.md`,
  no tooling reads or expects this file anymore. Left in place, opening
  this PR would reproduce the modify/delete conflict pattern that blocked
  #236/#237, against the current, explicit `main` policy — squarely a
  "not the stated change" file this round exists to catch.

**Verification re-run this round:** `npx vitest run` — 17 files / 36 tests,
all passing (re-ran after the fix below, not reused). `npx tsc --noEmit` —
clean.

**Findings:** one — `AGENT-SPEC.md` re-added stale root-level scaffolding
that `main` had deliberately deleted one commit before this branch
recreated it, duplicating `.agent/XAL-1161/SPEC.md` and reproducing a
known merge-conflict pattern. `AGENT-GOAL.md` is legitimate, tool-owned
scaffolding with its own deletion step already covered elsewhere; left
untouched.

**Changes made this round:** removed `AGENT-SPEC.md` (`git rm`), restoring
this branch to `main`'s current scaffolding policy. No product/content
files touched.
