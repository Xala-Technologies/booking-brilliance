...
2. **`server/nginx.snippet.conf`** — extend the digilist.no server-block
   ...
   - Validated with `nginx -t` against a throwaway `http{}`/`server{}`
     wrapper that `include`s this file — syntax is clean. (No `.conf`
     automated test exists in this repo for nginx snippets; this is the same
     verification depth PR #106 used.)

3. **`src/components/MobileMenu.test.tsx`** (new) — pins the drawer's logo to
   `/logo-64.webp` so a future edit can't silently reintroduce the 108KB
   asset.
...
layout-forcing property, and does so on a `setInterval` for as long as the
tab is open. This — not the MobileMenu asset — is why local TTI was 18.6s
before this change (Lighthouse's TTI algorithm needs a 5s window with no
long tasks; a task every 2.8s forever prevents that window from ever
naturally occurring) and is still 6.8s after. Not fixed here: it isn't one of
the ticket's named findings, and `RotatingWord` is a heavily-reviewed,
WCAG-motion-aware component (own file-header comment documents 4 prior
constraints, including an explicit — and, per this investigation, only
partially effective — CLS mitigation already attempted). Changing its
animation technique (e.g. to a composited transform) is a real fix but a
separate, riskier change; noted as a follow-up, not attempted in this
smallest-change pass.

## BLAST RADIUS

- `MobileMenu` is imported once, by `Navbar.tsx:6` (`import { MobileMenu }
  from "./MobileMenu"`), which is rendered on every page (checked: no other
  importer of `MobileMenu`, `grep -rn "MobileMenu" src/`). The changed `<img>`
  is `aria-hidden="true"`/`alt=""` (decorative, not read by assistive tech)
  and keeps the same `width`/`height`/`className` — no layout or a11y change,
  purely a smaller asset already proven safe by its use in `Navbar`/`Footer`.
- `server/nginx.snippet.conf` is not consumed by any code in this repo
  (`grep -rln "nginx.snippet"` → only this file and its own header comment)
  — it's a manually-applied ops doc, so this change has zero effect until
  someone pastes it into the live VPS config and reloads nginx. No CI/build
  step reads it.
- No other file references `/logo.svg` for this decorative-mark use case
  besides the ones already on `/logo-64.webp` (`Navbar.tsx`, `Footer.tsx`);
  `logo.svg` is still used elsewhere (`SEO.tsx:150` org schema `logo` field,
  `HeroPlatformPreview.tsx:209`) and is untouched — those aren't the "always
  loaded but invisible" case this fix addresses.
