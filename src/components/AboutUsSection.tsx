...
const AboutUsSection = () => {
  // Rendered at /om-oss and /en/om-oss by the same route tree, so the copy has
  // to follow the URL. Before this the English page carried the Norwegian
  // colophon in full — 14 Norwegian runs, which is what check:english caught.
  const locale = localeFromPath(useLocation().pathname);
  const copy = aboutCopy(locale);
  const { facts, timeline, creed } = copy;
  return (
    <section id="om-oss" className="py-16 lg:py-24 bg-paper-tinted border-y border-rule">
      <div className="container mx-auto md:px-8 lg:px-12">
        <SectionRule label={copy.rule} />
        ...
        >
          ...
          <div className="grid lg:grid-cols-12 gap-6 lg:gap-gutter py-10 lg:py-16">
            ...
            <div className="lg:col-span-9">
              ...
              >
                {copy.quoteLead}{" "}
                <em className="italic">{copy.quoteEm}</em>
                {copy.quoteTail}
              </blockquote>
